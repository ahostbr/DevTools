...
- `FSOTS_BPGenGraphSpec`
    ...
    You can also specify a top-level string such as
    `"GraphSpecFile": "bpgen_specs/MyJob_graph.json"` (or the alias
    `GraphSpecPath`). If present, the Commandlet will use that external
    JSON file instead of looking for an inline `GraphSpec` object.
...
- Logs now include the Commandlet stdout/stderr so link-validation warnings
    emitted by the hardened builder are preserved for manual inspection.
