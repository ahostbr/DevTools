...
def _extract_header_segments(header: dict) -> tuple[str, str, str]:
    """Extract (category, plugin, pass) from a parsed header dict.
    ...
    """
    # Category
    raw_category = header.get("category")
    category = sanitize_segment(raw_category, "uncategorized")
    ...
    return category, plugin, pass_segment
...
def route_file(file_path: str, inbox_dir: str, dry_run: bool = False):
    ...
    if header is None:
        # No header or parse error – we do not move the file, just report.
        return (file_path, None, f"SKIP: {err}")

    category, plugin, pass_segment = _extract_header_segments(header)

    dest_dir = os.path.join(inbox_dir, category, plugin, pass_segment)
    ...
    if dry_run:
        reason = (
            ...
            f"[category={category}, plugin={plugin}, pass={pass_segment}]"
        )
